...
def test_types_read_csv() -> None:
    ...
    with tempfile.NamedTemporaryFile() as file:
        ...
        df8: pd.DataFrame = pd.read_csv(file.name, header=0, usecols=['col1', 'col2'])
        df9: pd.DataFrame = pd.read_csv(file.name, header=0, usecols=[0, 1])
        df10: pd.DataFrame = pd.read_csv(file.name, doublequote=True, quotechar='"', on_bad_lines='skip')
        df11: pd.DataFrame = pd.read_csv(file.name, compression='infer', decimal='.', low_memory=True)
